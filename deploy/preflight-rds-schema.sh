...
assert_no_missing_requirements \
    ...
    "SELECT requirement
       FROM (
           ...
           UNION ALL SELECT 'fabrics.movement_policy', EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'fabrics' AND column_name = 'movement_policy')
           ...
           UNION ALL SELECT 'columns.movement_policy_constraints', 4 = (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND ((table_name = 'fabrics' AND ((column_name = 'movement_policy' AND column_type = 'varchar(20)' AND is_nullable = 'NO') OR (column_name = 'display_order' AND column_type = 'int' AND is_nullable = 'NO'))) OR (table_name IN ('walls', 'pillars') AND column_name = 'display_order' AND column_type = 'int' AND is_nullable = 'NO')))
           ...
           UNION ALL SELECT 'constraints.layout_zone_integrity', 17 = (SELECT COUNT(DISTINCT constraint_name) FROM information_schema.table_constraints WHERE constraint_schema = DATABASE() AND constraint_name IN ('uk_fabrics_id_version', 'ck_fabrics_movement_policy', 'uk_walls_id_version', 'uk_pillars_id_version', 'uk_layout_zones_id_version', 'uk_layout_zones_version_name', 'ck_layout_zones_extent', 'fk_layout_zones_layout_version', 'fk_layout_zones_default_exit', 'uk_layout_zone_members_wall', 'uk_layout_zone_members_pillar', 'uk_layout_zone_members_fabric', 'ck_layout_zone_members_exactly_one', 'fk_layout_zone_members_zone', 'fk_layout_zone_members_wall', 'fk_layout_zone_members_pillar', 'fk_layout_zone_members_fabric'))
           UNION ALL SELECT 'constraint.ck_fabrics_movement_policy', EXISTS(SELECT 1 FROM information_schema.table_constraints WHERE constraint_schema = DATABASE() AND table_name = 'fabrics' AND constraint_name = 'ck_fabrics_movement_policy' AND constraint_type = 'CHECK')
